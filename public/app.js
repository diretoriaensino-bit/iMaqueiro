...
function enviarDefeito() {
    ...
    let defeitos = JSON.parse(localStorage.getItem('imaqueiro_defeitos')) || [
        { equip: 'Maca Padrão', obs: 'Grade lateral não trava', autor: 'Sistema' }
    ];
    ...
    document.getElementById('toast-autor').innerText = 'Manutenção';
    document.getElementById('toast-msg').innerText = `Relatório enviado!`;
...
function carregarManutencao() {
    ...
    let defeitos = JSON.parse(localStorage.getItem('imaqueiro_defeitos')) || [
        { equip: 'Cadeira de Rodas', obs: 'Apoio de pé solto. Risco de queda.', autor: 'Marcos' }
    ];
    ...
    if(defeitos.length === 0) {
        ...
        defeitos.forEach((d, index) => {
            ...
            <div id="defeito-${index}" style="padding: 15px; border-left: 4px solid var(--danger); background: var(--input-bg); border-radius: 8px; margin-bottom: 12px; border: 1px solid var(--border-color); position: relative; transition: 0.3s;">
                <button onclick="resolverDefeito(${index})" style="position:absolute; right:10px; top:10px; background:var(--success-light); border:1px solid var(--success); color:var(--success-dark); border-radius: 8px; padding: 6px 10px; cursor:pointer; font-size: 0.85rem; font-weight: 700; transition: 0.2s;" title="Marcar como Consertado">
                    <i class="fa fa-check"></i> Reparado
                </button>
...
window.onload = () => { 
    try { const salvo = localStorage.getItem('imaqueiro_user'); if (salvo) { usuario = JSON.parse(salvo); socket.emit('relogar_maqueiro', usuario); abrirPainel(usuario); } } catch(e) { console.error("Erro ao carregar usuário salvo", e); localStorage.removeItem('imaqueiro_user'); }
};
...
function toggleStatusMaqueiro() {
    ...
    if (card.classList.contains('status-ocupado')) {
        return Swal.fire({
            icon: 'warning',
            title: 'Em Rota',
            text: 'Você não pode entrar em intervalo enquanto estiver com um transporte em andamento!',
            toast: true,
            position: 'top-end',
...
function filtrarTabelaADM() { const t = document.getElementById('adm-search').value.toLowerCase(); document.querySelectorAll('#adm-live-table tbody tr').forEach(r => r.style.display = r.innerText.toLowerCase().includes(t) ? '' : 'none'); }
...
function filtrarTabelaADM(filtro, botao) {
    ...
    if (botao) {
        if (filtro === 'vermelhos' || filtro === 'atrasos') botao.classList.add('active-danger');
        else botao.classList.add('active');
    }
...
function aplicarFiltroTabelaADM() {
    document.querySelectorAll('.adm-row-item').forEach(tr => {
        let show = false;
        let urgencia = tr.getAttribute('data-urgencia');
        ...
        ativos.forEach(p => {
            ...
            if(usuario.cargo === 'enfermagem' && nList) {
                ...
                if (p.status === 'pendente' || p.status === 'agendado') { 
                    ...
                    c1 = 'completed'; c2 = 'active'; 
                    icon2 = 'fa-running fa-flip'; 
                    progress = 33; 
                }
                else if (p.status.includes('na_origem')) { 
                    c1 = 'completed'; c2 = 'completed'; c3 = 'active'; 
                    icon3 = 'fa-procedures fa-fade'; 
                    progress = 66; 
                }
                else if (p.status.includes('em_transito')) { 
                    c1 = 'completed'; c2 = 'completed'; c3 = 'completed'; c4 = 'active'; 
                    icon4 = 'fa-ambulance fa-shake'; 
                    progress = 85; 
                }
                else if (p.status.includes('no_destino')) { 
                    c1 = 'completed'; c2 = 'completed'; c3 = 'completed'; c4 = 'completed'; 
                    progress = 100; 
                }

                let statusBox = `
                <div class="hospital-tracker">
                    <div class="track-line-bg"></div>
                    <div class="track-line-fill" style="width: ${progress}%;"></div>
                    
                    <div class="track-step ${c1}">
                        <div class="track-icon"><i class="fa ${icon1} modern-icon"></i></div>
                        <div class="track-label">Pedido</div>
                    </div>
                    <div class="track-step ${c2}">
                        <div class="track-icon"><i class="fa ${icon2} modern-icon"></i></div>
                        <div class="track-label">A Caminho</div>
                    </div>
                    <div class="track-step ${c3}">
                        <div class="track-icon"><i class="fa ${icon3} modern-icon"></i></div>
                        <div class="track-label">No Leito</div>
                    </div>
                    <div class="track-step ${c4}">
                        <div class="track-icon"><i class="fa ${icon4} modern-icon"></i></div>
                        <div class="track-label">Entregue</div>
                    </div>
                </div>
                <div style="text-align:center; color:var(--primary-hover); font-size:0.9rem; font-weight:800; margin-top:12px; background: var(--primary-light); padding: 10px; border-radius: 8px;">
                    <i class="fa fa-info-circle modern-icon"></i> ${statusText}
                </div>`;
            ...
            if(usuario.cargo === 'maqueiro') {
                ...
                if(isPendente && mPendentes) {
                    ...
                    let btnColor = "#CBD5E1";
                    let badgePronto = `<span class="badge badge-preparo" style="margin-bottom:8px;">EM PREPARO</span><br>`;
                    ...
                    if(pacienteEstaPronto && souOSugerido && !chamadosRejeitados.includes(p.id) && usuario.status_trabalho !== 'intervalo') {
                    ...
                    mPendentes.innerHTML += `${cardBase}${badgePronto}<button class="btn btn-main" style="${estiloBotao} margin-top:15px; width:100%;" onclick="aceitarChamadoManual(${p.id})" ${pacienteEstaPronto && souOSugerido ? '' : 'disabled'}><i class="fa fa-hand-paper modern-icon"></i> ${btnText}</button></div>`;
                } 
...
setInterval(() => {
    document.querySelectorAll('.sla-timer').forEach(el => {
        const statusText = el.closest('.card').innerText.toLowerCase();
        
        if (statusText.includes('aguardando') || statusText.includes('falta equip') || statusText.includes('agendado')) {
            el.className = 'sla-timer sla-badge sla-paused';
            el.innerHTML = `<i class="fa fa-pause modern-icon"></i> PAUSADO`;
            return;
        }
        
        const startTime = new Date(el.getAttribute('data-time')).getTime(); 
        const diffMins = Math.floor((Date.now() - startTime) / 60000);
        el.innerHTML = `<i class="fa fa-clock modern-icon"></i> ${diffMins} min`;
        if (diffMins >= 20) { el.className = 'sla-timer sla-badge sla-danger'; } else if (diffMins >= 10) { el.className = 'sla-timer sla-badge sla-warning'; } else { el.className = 'sla-timer sla-badge'; }
    });
}, 10000);
...
function simularTasy() {
    ...
    if (!prontuario) {
        return Swal.fire('Atenção', 'Digite o número do prontuário antes de buscar.', 'warning');
    }

    Swal.fire({
        title: 'Consultando Tasy...',
        html: 'Buscando dados do paciente e leito.',
...
socket.on('retorno_tasy', (resposta) => {
    if (resposta.sucesso) {
        ...
        Swal.fire({
            ...
            text: 'Prontuário não localizado no Tasy. Os campos foram liberados para digitação manual.'
        });
    }
});
...
async function emitirSOS(id) {
    const confirmacao = await Swal.fire({
        icon: 'error',
        title: '🚨 CÓDIGO AZUL',
        text: 'Isso acionará o ALARME GERAL na Coordenação e Enfermagem. O paciente teve uma parada ou piora súbita?',
        showCancelButton: true,
        confirmButtonText: 'EMERGÊNCIA! ACIONAR',
    ...
    if(confirmacao.isConfirmed) {
        socket.emit('alerta_sos', { id: id, maqueiro: usuario.nome });
    }
}
...
socket.on('sos_disparado', async data => {
    ...
    if (usuario.cargo === 'enfermagem' || usuario.cargo === 'coordenador') {
        const confirmacao = await Swal.fire({
            icon: 'error',
            title: '🚨 CÓDIGO AZUL 🚨',
            html: `<b style="font-size:1.2rem;">Maqueiro: ${data.maqueiro}</b><br>Solicitou resgate imediato no chamado #${data.id}!`,
            confirmButtonText: '<i class="fa fa-running"></i> CONFIRMAR RESGATE (Desligar Alarme)',
            confirmButtonColor: '#EF4444',
            allowOutsideClick: false,
            allowEscapeKey: false,   
            backdrop: `rgba(239, 68, 68, 0.5)` 
        });
        ...
        document.getElementById('toast-autor').innerText = '🚨 CÓDIGO AZUL 🚨';
        document.getElementById('toast-msg').innerHTML = `<b style="font-size:1.1rem;">Maqueiro: ${data.maqueiro} solicitou resgate!</b>`;
        document.getElementById('toast').style.display = 'flex';
        document.getElementById('toast').style.background = 'var(--danger)';
        document.getElementById('toast').style.borderLeft = '10px solid #7F1D1D';
...
socket.on('sos_aceito', data => {
    ...
    if (usuario.nome === data.maqueiro) {
        Swal.fire({
            ...
            html: `A enfermagem (<b>${data.enfermeiro}</b>) confirmou o Código Azul e está correndo para o local!`,
            confirmButtonColor: '#10B981',
        ...
        Swal.fire({
            ...
            position: 'top-end',
            icon: 'info',
            title: `Resgate assumido por ${data.enfermeiro}`,
            showConfirmButton: false,
            timer: 4000
        });
    }
